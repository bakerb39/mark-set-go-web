(() => {
  ...
  function readStoredObjectSafe(key) {
    try {
      const value = JSON.parse(localStorage.getItem(key) || '{}');
  ...
  function workspaceData() {
    ...
    const title = String(current.title || getBookContext().title || 'Current reading');
    const activity = readStoredArraySafe('markSetGoReadingActivityV1');
    const comprehension = readStoredArraySafe('markSetGoComprehensionV1');
    const progressMap = readStoredObjectSafe('markSetGoReadingProgressV1');
    const progress = progressMap[documentId] || {};
    const docActivity = activity.filter((row) => String(row.documentId || '') === documentId);
    const docComprehension = comprehension.filter((row) => String(row.documentId || '') === documentId);
    const seconds = docActivity.reduce((sum,row)=>sum+(Number(row.seconds)||0),0);
    const words = docActivity.reduce((sum,row)=>sum+(Number(row.wordsRead)||0),0);
    const averageWpm = seconds ? Math.round(words / (seconds / 60)) : Number(document.querySelector('#speed')?.value || 0) || 0;
    const comprehensionAverage = docComprehension.length
      ? Math.round(docComprehension.reduce((sum,row)=>sum+(Number(row.scorePercent)||0),0)/docComprehension.length)
      : 0;
    const effectiveWpm = averageWpm && comprehensionAverage ? Math.round(averageWpm * comprehensionAverage / 100) : 0;
    const percent = Number(progress.totalWords)
      ? Math.min(100,Math.round((Number(progress.furthestWord)||0)/Number(progress.totalWords)*100))
      : 0;
    const wpmGoal = Math.max(50, Number(localStorage.getItem('markSetGoReadingWpmGoalV1')) || 300);
    const comprehensionGoal = Math.max(1, Math.min(100, Number(localStorage.getItem('markSetGoReadingComprehensionGoalV1')) || 85));
    const year = new Date().getFullYear();
    const months = Array.from({length:12},(_,month)=>({month,label:new Date(year,month,1).toLocaleDateString(undefined,{month:'short'}),wpm:0,comprehension:0}));
    months.forEach((row)=>{
      const matching=activity.filter((item)=>{
        const d=new Date(item.endedAt || item.createdAt || 0);
        return d.getFullYear()===year && d.getMonth()===row.month;
      });
      const monthSeconds=matching.reduce((sum,item)=>sum+(Number(item.seconds)||0),0);
      const monthWords=matching.reduce((sum,item)=>sum+(Number(item.wordsRead)||0),0);
      row.wpm=monthSeconds?Math.round(monthWords/(monthSeconds/60)):0;
      const checks=comprehension.filter((item)=>{
        const d=new Date(item.createdAt || item.endedAt || 0);
        return d.getFullYear()===year && d.getMonth()===row.month;
      });
      row.comprehension=checks.length?Math.round(checks.reduce((sum,item)=>sum+(Number(item.scorePercent)||0),0)/checks.length):0;
    });
    return { current, documentId, title, activity, comprehension, progress, docActivity, docComprehension, seconds, words, averageWpm, comprehensionAverage, effectiveWpm, percent, wpmGoal, comprehensionGoal, months, year };
  }
  ...
  function workspaceLineChart(data, key, goal, { suffix='', maxValue=null }={}) {
    const width=520,height=210,left=42,right=18,top=18,bottom=34;
    const values=data.map(row=>Number(row[key])||0);
    const max=Math.max(maxValue || 0, goal || 0, ...values, 1);
    const usableW=width-left-right, usableH=height-top-bottom;
    const pointFor=(row,index)=>({
      x:left+(data.length===1?usableW/2:index*usableW/(data.length-1)),
      y:top+usableH-((Number(row[key])||0)/max)*usableH,
      row
    });
    const points=data.map(pointFor);
    const nonzero=points.filter(p=>Number(p.row[key])>0);
    const path=nonzero.map((p,index)=>`${index?'L':'M'} ${p.x.toFixed(1)} ${p.y.toFixed(1)}`).join(' ');
    const area=nonzero.length>1 ? `${path} L ${nonzero[nonzero.length-1].x.toFixed(1)} ${(top+usableH).toFixed(1)} L ${nonzero[0].x.toFixed(1)} ${(top+usableH).toFixed(1)} Z` : '';
    const goalY=top+usableH-(goal/max)*usableH;
    return `<svg class="reader-workspace-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(key==='wpm'?'Reading speed progress':'Comprehension progress')}">
      <defs><linearGradient id="rw-${key}-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="currentColor" stop-opacity=".24"/><stop offset="100%" stop-color="currentColor" stop-opacity="0"/></linearGradient></defs>
      ${[0,.25,.5,.75,1].map(r=>{const y=top+usableH-r*usableH;return `<line class="rw-chart-grid" x1="${left}" y1="${y}" x2="${width-right}" y2="${y}"/><text class="rw-chart-label" x="${left-8}" y="${y+4}" text-anchor="end">${Math.round(max*r)}${suffix}</text>`}).join('')}
      <line class="rw-chart-goal" x1="${left}" y1="${goalY}" x2="${width-right}" y2="${goalY}"/><text class="rw-chart-goal-label" x="${width-right}" y="${Math.max(12,goalY-6)}" text-anchor="end">Goal ${goal}${suffix}</text>
      ${area?`<path class="rw-chart-area" d="${area}"/>`:''}
      ${path?`<path class="rw-chart-line" d="${path}"/>`:''}
      ${nonzero.map(p=>`<circle class="rw-chart-dot" cx="${p.x}" cy="${p.y}" r="4"><title>${escapeHtml(p.row.label)}: ${Number(p.row[key])||0}${suffix}</title></circle>`).join('')}
      ${points.filter((_,i)=>i%2===0).map(p=>`<text class="rw-chart-month" x="${p.x}" y="${height-10}" text-anchor="middle">${escapeHtml(p.row.label)}</text>`).join('')}
    </svg>`;
  }

  function workspaceProgressMarkup() {
    const d=workspaceData();
    const latestNote = d.docComprehension[0];
    return `<div class="reader-workspace-progress-inner">
      <section class="rw-companion-nudge">
        <img src="${companionAvatar()}" alt="${companionName()}">
        <div><span>Your reading companion</span><strong>${companionName()} is tracking your progress</strong><p>${d.comprehensionAverage ? `You’re averaging ${d.averageWpm || '—'} WPM with ${d.comprehensionAverage}% comprehension on this text.` : 'Keep reading and take a comprehension check when you are ready.'}</p></div>
      </section>
      <section class="rw-current-book-card">
        <div class="rw-book-mark" aria-hidden="true">▤</div>
        <div class="rw-current-title"><span>Current text</span><strong>${escapeHtml(d.title)}</strong><small>${d.docActivity.length} recorded session${d.docActivity.length===1?'':'s'}</small></div>
        <div class="rw-current-metrics">
          <div><strong>${d.percent}%</strong><span>Complete</span></div>
          <div><strong>${formatDurationShort(d.seconds)}</strong><span>Time</span></div>
          <div><strong>${d.averageWpm || '—'}</strong><span>WPM</span></div>
          <div><strong>${d.comprehensionAverage || '—'}${d.comprehensionAverage?'%':''}</strong><span>Comprehension</span></div>
          <div><strong>${d.effectiveWpm || '—'}</strong><span>Effective WPM</span></div>
        </div>
      </section>
      <div class="rw-progress-chart-grid">
        <section class="rw-progress-chart-card rw-wpm-card"><header><div><span>Speed over time</span><h3>WPM Progress</h3></div><b>Goal: ${d.wpmGoal} WPM</b></header>${workspaceLineChart(d.months,'wpm',d.wpmGoal,{suffix:''})}</section>
        <section class="rw-progress-chart-card rw-comprehension-card"><header><div><span>Understanding over time</span><h3>Comprehension</h3></div><b>Goal: ${d.comprehensionGoal}%</b></header>${workspaceLineChart(d.months,'comprehension',d.comprehensionGoal,{suffix:'%',maxValue:100})}</section>
      </div>
      <div class="rw-quick-actions">
        <button type="button" data-rw-action="companion"><span>✦</span><strong>Explain passage</strong></button>
        <button type="button" data-rw-action="quiz"><span>◎</span><strong>Take quiz</strong></button>
        <button type="button" data-rw-action="notes"><span>✎</span><strong>Save note</strong></button>
        <button type="button" data-action="progress-awards"><span>▥</span><strong>View full progress</strong></button>
      </div>
      ${latestNote?`<section class="rw-recent-highlight"><header><strong>Latest comprehension check</strong><span>${escapeHtml(new Date(latestNote.createdAt||Date.now()).toLocaleDateString())}</span></header><div><b>${Number(latestNote.scorePercent)||0}%</b><p>${escapeHtml(latestNote.title || d.title)}</p></div></section>`:''}
    </div>`;
  }

  function workspaceReadMarkup() {
    const d=workspaceData();
    return `<div class="reader-workspace-simple-view">
      <section class="rw-welcome-card"><span>Now reading</span><h3>${escapeHtml(d.title)}</h3><p>Your book stays open while the tools in this workspace come to you.</p></section>
      <div class="rw-read-kpis"><article><strong>${d.percent}%</strong><span>Complete</span></article><article><strong>${d.averageWpm||'—'}</strong><span>WPM</span></article><article><strong>${d.comprehensionAverage||'—'}${d.comprehensionAverage?'%':''}</strong><span>Comprehension</span></article></div>
      <section class="rw-reader-actions"><button type="button" data-rw-action="companion">✦ Ask ${companionName()}</button><button type="button" data-rw-action="progress">▥ View progress</button><button type="button" data-rw-action="contents">☰ Open contents</button></section>
    </div>`;
  }

  function workspaceNotesMarkup() {
    const d=workspaceData();
    return `<div class="reader-workspace-simple-view"><section class="rw-section-intro"><span>Notes</span><h3>Thinking alongside the text</h3><p>Keep notes and saved ideas close while you continue reading.</p></section><div class="rw-empty-friendly"><span>✎</span><strong>Your reading notes stay tied to this text.</strong><p>Use the existing note action on a word or passage, or open My Notebook for your full collection.</p><button type="button" data-action="mark-notebook">Open My Notebook</button></div></div>`;
  }

  function workspaceStudyMarkup() {
    return `<div class="reader-workspace-simple-view"><section class="rw-section-intro"><span>Study</span><h3>Turn reading into learning</h3><p>Practice understanding without leaving your current book.</p></section><div class="rw-study-grid"><button type="button" data-rw-action="quiz"><span>◎</span><strong>Comprehension quiz</strong><small>Check understanding</small></button><button type="button" data-action="mnemonics"><span>◇</span><strong>Mnemonics</strong><small>Build memory anchors</small></button><button type="button" data-read="syntopicon"><span>✦</span><strong>Great Ideas</strong><small>Connect ideas across books</small></button><button type="button" data-action="language-learning"><span>文</span><strong>Language learning</strong><small>Practice through reading</small></button></div></div>`;
  }

  function workspaceContentsMarkup() {
    const nav=document.querySelector('#navigation-pane');
    const toc=nav?.querySelector('.toc-list')?.innerHTML || '<p class="navigation-empty">Contents will appear here when chapter headings are available.</p>';
    return `<div class="reader-workspace-simple-view"><section class="rw-section-intro"><span>Contents</span><h3>Move through this text</h3><p>Jump to a chapter without leaving the reader.</p></section><div class="rw-contents-clone">${toc}</div></div>`;
  }

  function workspaceMarkup() {
    return `<div class="reader-workspace" data-reader-workspace>
      <nav class="reader-workspace-tabs" role="tablist" aria-label="Reader workspace">
        <button type="button" class="is-active" role="tab" aria-selected="true" data-rw-view="read"><span>▤</span>Read</button>
        <button type="button" role="tab" aria-selected="false" data-rw-view="companion"><span>✦</span>Companion</button>
        <button type="button" role="tab" aria-selected="false" data-rw-view="notes"><span>✎</span>Notes</button>
        <button type="button" role="tab" aria-selected="false" data-rw-view="progress"><span>▥</span>Progress</button>
        <button type="button" role="tab" aria-selected="false" data-rw-view="study"><span>◎</span>Study</button>
        <button type="button" role="tab" aria-selected="false" data-rw-view="contents"><span>☰</span>Contents</button>
      </nav>
      <div class="reader-workspace-panels">
        <section class="reader-workspace-panel is-active" data-rw-panel="read">${workspaceReadMarkup()}</section>
        <section class="reader-workspace-panel reader-workspace-companion-panel" data-rw-panel="companion">${premiumMarkup()}</section>
        <section class="reader-workspace-panel" data-rw-panel="notes">${workspaceNotesMarkup()}</section>
        <section class="reader-workspace-panel" data-rw-panel="progress">${workspaceProgressMarkup()}</section>
        <section class="reader-workspace-panel" data-rw-panel="study">${workspaceStudyMarkup()}</section>
        <section class="reader-workspace-panel" data-rw-panel="contents">${workspaceContentsMarkup()}</section>
      </div>
    </div>`;
  }

  function activateWorkspaceView(view='read') {
    if (!shell) return;
    const valid=['read','companion','notes','progress','study','contents'];
    if(!valid.includes(view)) view='read';
    shell.querySelectorAll('[data-rw-view]').forEach((button)=>{
      const active=button.dataset.rwView===view;
      button.classList.toggle('is-active',active);
      button.setAttribute('aria-selected',String(active));
    });
    shell.querySelectorAll('[data-rw-panel]').forEach((panel)=>panel.classList.toggle('is-active',panel.dataset.rwPanel===view));
    shell.dataset.workspaceView=view;
    if(view==='progress') {
      const panel=shell.querySelector('[data-rw-panel="progress"]');
      if(panel) panel.innerHTML=workspaceProgressMarkup();
    }
    if(view==='read') {
      const panel=shell.querySelector('[data-rw-panel="read"]');
      if(panel) panel.innerHTML=workspaceReadMarkup();
    }
    if(view==='contents') {
      const panel=shell.querySelector('[data-rw-panel="contents"]');
      if(panel) panel.innerHTML=workspaceContentsMarkup();
    }
    bindWorkspaceActionButtons();
  }

  function ensureWorkspaceVisible() {
    const layout=document.querySelector('#reader-layout');
    const button=document.querySelector('#toggle-word-panel');
    if(!layout) return;
    if(layout.classList.contains('word-panel-hidden')) layout.classList.remove('word-panel-hidden');
    button?.setAttribute('aria-pressed','true');
    button?.classList.remove('pane-closed');
    const ask=document.querySelector('#toggle-mark-panel');
    ask?.classList.remove('pane-closed');
  }

  function bindWorkspaceActionButtons() {
    if(!shell) return;
    shell.querySelectorAll('[data-rw-action]').forEach((button)=>{
      if(button.dataset.rwBound==='1') return;
      button.dataset.rwBound='1';
      button.addEventListener('click',()=>{
        const action=button.dataset.rwAction;
        if(action==='companion') activateWorkspaceView('companion');
        else if(action==='progress') activateWorkspaceView('progress');
        else if(action==='notes') activateWorkspaceView('notes');
        else if(action==='contents') activateWorkspaceView('contents');
        else if(action==='quiz') window.MarkSetGoStartComprehension?.();
      });
    });
    shell.querySelectorAll('.rw-contents-clone [data-toc-index]').forEach((button)=>{
      if(button.dataset.rwTocBound==='1') return;
      button.dataset.rwTocBound='1';
      button.addEventListener('click',()=>{
        document.querySelector(`#navigation-pane [data-toc-index="${CSS.escape(String(button.dataset.tocIndex||''))}"]`)?.click();
      });
    });
  }

  function bindWorkspaceEvents() {
    if(!shell || shell.dataset.workspaceEventsBound==='1') return;
    shell.dataset.workspaceEventsBound='1';
    shell.querySelectorAll('[data-rw-view]').forEach((button)=>button.addEventListener('click',()=>activateWorkspaceView(button.dataset.rwView)));
    bindWorkspaceActionButtons();
  }
  ...
  function configureShell() {
    ...
    shell.insertAdjacentHTML('beforeend', workspaceMarkup());
    bindPremiumEvents();
    bindWorkspaceEvents();
    ensureWorkspaceVisible();
    activateWorkspaceView('read');

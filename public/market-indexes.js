(() => {
  ...
  function boot() {
    ...
    // Profile is rendered on demand. Re-check shortly after the existing
    // Profile navigation action runs; no MutationObserver is used.
    document.addEventListener('click', (event) => {
      if (!event.target.closest('[data-action="profile-preferences"]')) return;
      [0, 50, 150, 300, 600].forEach((delay) => {
        window.setTimeout(installSetting, delay);
      });
    });
...
(() => {
  const href = '/explorer-theme.css?v=1.0.2';
...
(() => {
  ...
  function scheduleProfileInstall() {
    [0, 50, 150, 300, 600].forEach((delay) => {
      window.setTimeout(installProfileControls, delay);
    });
  }

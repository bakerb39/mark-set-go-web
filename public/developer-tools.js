(() => {
  ...
  async function runRegressionSuite() {
    ...
    try {
      ...
      add('Global help button identity has one owner', !companionSrc.includes("qsa('.app-help-mark-button'") && helpSrc.includes("openButton.dataset.companionId = c.id") && helpSrc.includes("openImg.setAttribute('src', avatar)") ? 'pass':'fail', 'app-help-mark.js exclusively owns the floating help button label + avatar, preventing mixed Mark/Beth state.');
      ...
      add('Reader companion button label + avatar stay synchronized', companionSrc.includes("const readerCompanionButton = root.querySelector('#toggle-mark-panel')") && companionSrc.includes("applyAvatar(readerCompanionButton?.querySelector(':scope > img'))") ? 'pass':'fail', 'The Reader Ask Mark/Ask Beth button updates its text and avatar from the same selected companion state.');

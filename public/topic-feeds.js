(() => {
  ...
  function topicFeedStoryHeaderParts(reader = document.querySelector('#reader')) {
    ...
    // MSG Topic Feed header contract:
    // - no cream/card band at rest;
    // - Source/date/View original/share is visible only before article scrolling;
    // - once the article moves, keep only the compact article actions as the
    //   first-page text ceiling. No MutationObserver is used.
    if (reader.dataset.topicFeedHeaderScrollBound !== '1') {
      ...
      const syncHeaderScrollState = () => {
        ...
        const scrolled = (Number(reader.scrollTop) || 0) > 4;
        header.classList.toggle('topic-feed-story-header-scrolled', scrolled);

        if (meta?.isConnected) {
          meta.style.setProperty('display', scrolled ? 'none' : '', 'important');
        }
        ...
        if (liveActionRow) {
          ...
          liveActionRow.style.setProperty('width', '100%', 'important');
          liveActionRow.style.setProperty('max-width', '100%', 'important');
        }
      };

      reader.addEventListener('scroll', syncHeaderScrollState, { passive: true });
      syncHeaderScrollState();
    }

    // Always neutralize old card styling when this header is rebuilt.
  ...
  function stripTrailingTopicFeedProvenance(text) {
    ...
    // Remove feed-provenance artifacts from the article body. Source/date/link
    // belong only in the compact external Topic Feed header.
    return raw
      .replace(/\n{2,}\s*Source:\s*[^\n]+\nhttps?:\/\/\S+\s*$/i, '')
      .replace(/\n{2,}\s*Source:\s*[^\n]+(?:\n|\s)+https?:\/\/\S+(?:\s+SOURCE\s+[^\n]+)?\s*$/i, '')
      .trim();
  }

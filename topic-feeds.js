(() => {
  ...
  function positionTopicFeedStoryHeader() {
    ...
    // MSG fix: keep the external band physically separate from article text.
    // Do not move it back into #reader and do not add a MutationObserver.
    readerFrame.style.setProperty('position', 'relative');
    ...
    window.requestAnimationFrame(() => {
      ...
      const requiredHeight = Math.max(fontSize * 2.5, headerHeight + Math.max(14, fontSize * 1.25));
      const previousHeight = Number.parseFloat(spacer.style.height) || 0;
      spacer.style.display = 'block';
      spacer.style.flex = '0 0 auto';
      spacer.style.width = '100%';
      spacer.style.maxWidth = `${headerWidth}px`;
      spacer.style.minHeight = `${Math.ceil(requiredHeight)}px`;
  ...
  function openPreparedArticle(topic, article, payload, options = {}) {
    ...
    const documentRecord = {
      ...
      text: readerText || String(article.feedText || article.summary || '').trim() || 'Full article text could not be imported from the publisher. Choose View Original, then use the Read with Mark bookmarklet from Read Anything to import the page.',
      source: {
        type: 'topic-feed',
